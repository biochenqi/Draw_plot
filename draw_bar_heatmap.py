...
def draw_heatmap(df,group):
    ...
    g.ax_col_dendrogram.legend(loc='lower right', bbox_to_anchor=(1.5, -4), ncol=1,title='ROIs')
    #去除掉热图的x坐标轴标签
    ...
    # g.ax_col_colors.set_ylabel('ROIs')
    #设置bar的位置大小
    ...
    # g.ax_cbar.set_title('colorbar')
    #文件保存
    plt.savefig('heatmap.png',dpi=100,bbox_inches='tight')
    plt.savefig('heatmap.pdf',dpi=100,bbox_inches='tight')
...
def draw_bowplot(df,list_type,prefix):
    ...
    for i, cell_type in enumerate(x_labels):
        ...
        plt.text(i-0.3, y+4, '{:.3f}'.format(p),fontsize=6,color='red' if p<0.05 else 'black')
        # 添加注释到图形中
        # if p < 0.05:
    ...
    plt.savefig('%s_boxplot.png'%prefix,dpi=300,bbox_inches='tight')
    plt.savefig('%s_boxplot.pdf'%prefix,dpi=300,bbox_inches='tight')
...
def draw_bowplot_alltype(df,prefix):
    ...
    add_stat_annotation(ax, data=df, x="cell_type", y="value", hue="group2",
                    ...
                    loc='inside', verbose=2,comparisons_correction=None,hue_order = hue_order)
    sns.despine(offset=10, trim=True)
    #设置X和Y轴label
    plt.xlabel('Cell type')
    plt.ylabel('Relative Abundance')
    ...
    plt.savefig('%s_boxplot_all.png'%prefix,dpi=300,bbox_inches='tight')
    plt.savefig('%s_boxplot_all.pdf'%prefix,dpi=300,bbox_inches='tight')
...
def draw_bar(df,group):
    ...
    plt.savefig('cell_ratio.png',dpi=300,bbox_inches='tight')
    plt.savefig('cell_ratio.pdf',dpi=300,bbox_inches='tight')
